...
@section('content')
    @include('admin.partials.header', ['title' => __('Edit SubCategory')])
                        ...
                        <form method="post" action="{{ route('subCategory.update', $subCategory->id) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('category_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="category_id" id="input-name" class="form-control form-control-alternative{{ $errors->has('category_id') ? ' is-invalid' : '' }}" placeholder="{{ __('Category') }}"required autofocus>
                                        @if($categories)
                                            @foreach($categories as $category)
                                                <option value="{{$category->id}}"
                                                        @if ($category->name == old('category', $subCategory->category->name))selected="selected"
